...
class QuickStudentRegistrationService
{
    ...
    {
        $this->modePolicy->resolve((int) ($data['enrollment_mode_id'] ?? 0));
        $this->feePolicy->assertEligibleIds(collect($data['services'] ?? [])->pluck('fee_id'), 'services');
        ...
        try {
            $result = DB::transaction(function () use ($data, $actor, $operationKey, $payloadHash, $outerToken) {
                ...
                $mode = $this->modePolicy->resolve((int) $data['enrollment_mode_id']);
