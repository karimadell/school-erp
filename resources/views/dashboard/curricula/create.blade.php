...
<div class="ui2-scope mx-auto max-w-2xl">
    ...
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
            <h1 class="text-2xl font-semibold text-slate-900">{{ __('curriculum.create') }}</h1>
            <p class="text-sm text-slate-500">{{ __('curriculum.create_hint') }}</p>
        </div>
    ...
    <div class="ui2-card">
        <form method="POST" action="{{ route('dashboard.curricula.store') }}">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="subject_id" name="subject_id"
                        ...
                        @foreach($subjects as $subject)
                            <option value="{{ $subject->id }}" @selected(old('subject_id') == $subject->id)>{{ $subject->name_ru }}</option>
                        @endforeach
                    </select>
                    @error('subject_id')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="number" id="weekly_hours" name="weekly_hours" min="1" value="{{ old('weekly_hours') }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <select id="type" name="type"
                        ...
                        <option value="">—</option>
                        <option value="mandatory" @selected(old('type') === 'mandatory')>{{ __('curriculum.type_mandatory') }}</option>
                        <option value="elective" @selected(old('type') === 'elective')>{{ __('curriculum.type_elective') }}</option>
                        <option value="optional_enrichment" @selected(old('type') === 'optional_enrichment')>{{ __('curriculum.type_optional_enrichment') }}</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
