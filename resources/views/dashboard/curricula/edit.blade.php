            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="assessment_type" name="assessment_type"
                        ...
                        <option value="grade" @selected(old('assessment_type', $curriculum->assessment_type ?? 'grade') === 'grade')>{{ __('curriculum.assessment_grade') }}</option>
                        <option value="pass_fail" @selected(old('assessment_type', $curriculum->assessment_type) === 'pass_fail')>{{ __('curriculum.assessment_pass_fail') }}</option>
                        <option value="ungraded" @selected(old('assessment_type', $curriculum->assessment_type) === 'ungraded')>{{ __('curriculum.assessment_ungraded') }}</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" id="report_order" name="report_order" min="1" max="999"
                           value="{{ old('report_order', $curriculum->report_order) }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <label class="flex items-center gap-2 text-sm text-slate-700">
                        <input type="checkbox" name="is_active" value="1" @checked(old('is_active', ($curriculum->is_active ?? true) ? '1' : '0') == '1')>
                ...
                <div class="sm:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="4" maxlength="2000"
                              class="ui2-input @error('notes') is-invalid @enderror">{{ old('notes', $curriculum->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
