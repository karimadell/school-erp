...
<div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h3 class="mb-1">{{ __('expenses.create') }}</h3>
        </div>
    ...
    <div class="card shadow-sm border-0">
        <div class="card-body p-4">
            <form method="POST" action="{{ route('dashboard.finance.expenses.store') }}" enctype="multipart/form-data">
                @csrf
                @include('dashboard.finance.expenses._form', ['expense' => null])

                <hr class="my-4">
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-success px-4">{{ __('expenses.save') }}</button>
                    <a href="{{ route('dashboard.finance.expenses.index') }}" class="btn btn-outline-secondary px-4">{{ __('expenses.cancel') }}</a>
                </div>
            </form>
        </div>
    </div>
