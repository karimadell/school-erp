    ...
    <div class="tab-content">
        ...
        <section id="transport-vehicles" class="tab-pane fade">
            ...
            <div class="card"><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Транспорт</th><th>Госномер</th><th>Места</th><th>Текущие маршруты</th><th>Сотрудники на дату</th><th>Статус</th><th></th></tr></thead><tbody>
                ...
                @php
                    $used=(int)($occupiedByBus[$bus->id]??0); $limit=min(14,$bus->student_capacity); $physical=(int)($physicalByBus[$bus->id]??$used);
                ...
                <tr>
                    ...
                    <td><span class="badge bg-{{ $used >= $limit ? 'danger' : 'success' }}">Ученики {{ $used }}/{{ $limit }}</span><div class="small text-{{ $physical >= $bus->passenger_capacity ? 'danger' : 'muted' }}">Пассажиры {{ $physical }}/{{ $bus->passenger_capacity }}</div></td>
                    <td>{{ $currentRoutes->implode(', ') ?: '—' }}</td><td>@forelse($currentStaff as $item)<div class="small">{{ $roleLabels[$item->role] }}: {{ $item->identity_name }}</div>@empty—@endforelse @if($bus->driver_name)<div class="small text-muted">Архивный водитель: {{ $bus->driver_name }}</div>@endif</td>
        ...
        <section id="transport-students" class="tab-pane fade">
            @if($canAssignments)<div class="card mb-3"><div class="card-header fw-semibold">Назначить транспорт ученику</div><div class="card-body"><form method="POST" action="{{ route('dashboard.transport-management.student-assignments.store') }}" class="row g-2 align-items-end">@csrf
                ...
                <div class="col-lg-2"><label class="form-label">Транспорт *</label><select name="bus_id" required class="form-select"><option value="">—</option>@foreach($activeBuses as $bus) @php $used=(int)($occupiedByBus[$bus->id]??0);$limit=min(14,$bus->student_capacity);$physical=(int)($physicalByBus[$bus->id]??$used); @endphp<option value="{{ $bus->id }}" @disabled($used >= $limit || $physical >= $bus->passenger_capacity)>{{ $bus->vehicle_code ?: $bus->name }} — {{ $used }}/{{ $limit }} students; {{ $physical }}/{{ $bus->passenger_capacity }} passengers{{ $used >= $limit?' (мест нет)':' ('.($limit-$used).' свободно)' }}</option>@endforeach</select></div>
