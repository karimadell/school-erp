 ...
 * 2026_09_04_090000_create_payment_refund_allocation_coverage_periods_table:
 * this table's own long name pushes THREE of Laravel's auto-generated
 * identifiers on it past MySQL's 64-character limit — not only the one
 * originally reported:
 *
 *   - payment_refund_allocation_id's FK constraint:      79 chars
 *   - installment_coverage_period_id's FK constraint:    81 chars
 *   - the explicit index on installment_coverage_period_id: 79 chars
 *
 * The composite unique (payment_refund_allocation_id,
 * installment_coverage_period_id) was already given an explicit short
 * name ('pr_alloc_cov_period_unique', 26 chars) by this migration's
 ...
 * the two prior migrations of this same corrective series), so a length
 ...
 */
class PaymentRefundAllocationCoveragePeriodSchemaTest extends TestCase
{
    use RefreshDatabase;

    public function test_both_foreign_keys_exist_with_correct_columns_and_targets(): void
    {
        $foreignKeys = collect(Schema::getForeignKeys('payment_refund_allocation_coverage_periods'));

        $refundAllocationFk = $foreignKeys->first(fn (array $fk) => $fk['columns'] === ['payment_refund_allocation_id']
            && $fk['foreign_table'] === 'payment_refund_allocations'
            && $fk['foreign_columns'] === ['id']);
        $this->assertNotNull($refundAllocationFk, 'expected a foreign key from payment_refund_allocation_id to payment_refund_allocations.id');
     ...
     * never asserting arbitrary full-file text.
     */
    public function test_both_foreign_key_constraint_names_are_short_and_explicit(): void
    {
        $source = file_get_contents(database_path('migrations/2026_09_04_090000_create_payment_refund_allocation_coverage_periods_table.php'));

        $refundAllocationFkName = $this->explicitConstrainedName($source, 'payment_refund_allocation_id', 'payment_refund_allocations');
        $this->assertNotNull($refundAllocationFkName,
            "payment_refund_allocation_id's foreignId()->constrained('payment_refund_allocations', 'id', <explicit name>) call must supply an explicit third (name) argument");
        $this->assertLessThanOrEqual(64, strlen($refundAllocationFkName),
            "FK constraint name '{$refundAllocationFkName}' exceeds MySQL's 64-character identifier limit");
    ...
    {
        $indexes = collect(Schema::getIndexes('payment_refund_allocation_coverage_periods'));

        $compositeUnique = $indexes->first(fn (array $index) => $index['unique']
            && $index['columns'] === ['payment_refund_allocation_id', 'installment_coverage_period_id']);
        $this->assertNotNull($compositeUnique, 'expected a unique index over (payment_refund_allocation_id, installment_coverage_period_id)');
        ...
        // ->index() call in this migration (unlike payment_refund_allocation_id,
        ...
        $autoGeneratedIndexName = 'payment_refund_allocation_coverage_periods_installment_coverage_period_id_index';
    ...
    public function test_original_auto_generated_fk_names_would_have_exceeded_the_limit(): void
    {
        ...
        $autoGeneratedRefundAllocationFk = 'payment_refund_allocation_coverage_periods_payment_refund_allocation_id_foreign';
        $this->assertGreaterThan(64, strlen($autoGeneratedRefundAllocationFk),
            'sanity check: this is what the auto-generated FK name would have been for payment_refund_allocation_id, and why it is unsafe on MySQL');

        $autoGeneratedInstallmentCoverageFk = 'payment_refund_allocation_coverage_periods_installment_coverage_period_id_foreign';
    ...
    {
        $columns = collect(Schema::getColumns('payment_refund_allocation_coverage_periods'))->keyBy('name');

        $this->assertFalse($columns['amount']['nullable']);
        $this->assertFalse($columns['created_at']['nullable']);
        $this->assertFalse($columns['payment_refund_allocation_id']['nullable']);
